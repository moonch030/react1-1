...
#### 15.1 CSS란?
- CSS는 스타일링을 위한 언어이다. 
- Cascading이란 계단식이란 뜻으로 한 엘리먼트에 여러 스타일이 적용될 경우 `스타일간의 충돌을 막기 위해 계단식`으로 스타일을 적용시키는 규칙을 갖고 있다.
- 즉 하나의 스타일이 `여러개의 엘리먼트에 적용될 수도있고, 하나의 엘리먼트에도 여러개의 스타일이 적용될 수도`있습니다.
...
- block은 세로로 정렬되며, with의 height를 가질 수 있다. 크기와 상관 없이 한 줄을 점유한다.
- inline은 가로로 정렬되며, with의 height를 가질 수 없다 컨텐츠의 크기만큼 공간을 점유한다.
---
### 2024-06-11 강의내용
...
#### 13.1 합성에 대해 알아보기
- 합성(Compostition)은 '여러개의 컴포넌트를 합쳐서 새로운 컴포넌트를 만드는 것' 입니다.   
- 조합 방법에 따라 합성의 사용 기법은 다음과 같이 나눌 수 있음.


[1] Containment(담다, 포함하다, 격리하다)
- 특정 컴포넌트는 `하위 컴포넌트를 포함하는 형태의 합성 방법`   
...
#### 10.1 리스트와 키란 무엇인가?
- 리스트는 자바스크립트의 변수나 객체를 하나의 변수로 묶어 놓은 배열과 같은것.   
- 키는 각 객체나 아이템을 구분할 수 있는 고유한 값을 의미   
- 리액트에서는 배열과 키를 사용하는 반복되는 다수의 엘리먼트를 쉽게 렌더링할 수 있음   

#### 10.2 여러개의 컴포넌트 사용하기
- 예의 에어비앤비의 화면처럼 같은 컴포넌트를 화면에 반복적으로 나타내야 할 경우 배열에 들어있는 엘리먼트를 `map()`함수를 이용하여 랜더링   
- 다음은 numbers배열에 들어있는 각각의 요소를 `map()`를 이용하여 하나씩 추출하여, 2를 곱한 후 double라는 배열에 다시 넣는 코드입니다.
```
01 const doubled = numbers.map((number) => number * 2);
```
- 다음은 리액트에서 `map()` 함수를 사용한 예제입니다.
```
01 const numbers=[1,2,3,4,5];
...
### 엘리먼트
#### 엘리먼트 변수
- 렌더링해야 될 컴포넌트를 변수처럼 사용하는 방법이 엘리멘트 변수   
- 277~278페이지 코드 처럼 state에 따라 button 변수에 컴포넌트의 객체를 저장하여 return문에서 사용   
...
- 첫 번 조건이 거짓이면 두번째 조건은 판단할 필요가 없다. 단축평가   
